...
--*/
{
    ...
    {
        //
        // For all capture streams we either source data from the VM mic ring buffer
        // (MicIn) or output silence. No tone generator is used for capture in the PoC.
        //
    }
...
)
{
    ...
    if (m_bCapture)
    {
        // Capture streams: for MicIn we source data from the VM mic ring buffer,
        // other capture endpoints keep the original tone-generator behavior.
...
--*/
{
    ...
    {
        //
        // Capture streams: MicIn uses the VM mic ring buffer, all other capture
        // endpoints output silence. This completely disables the tone generator
        // for capture in the PoC.
        //
        while (ByteDisplacement > 0)
        {
            ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);

            if (m_pMiniport != NULL && m_pMiniport->GetDeviceType() == eMicInDevice)
            {
                ULONG bytesRead = VmMicRead(m_pDmaBuffer + bufferOffset, runWrite);
                if (bytesRead < runWrite)
                {
                    ULONG remaining = runWrite - bytesRead;
                    RtlZeroMemory(m_pDmaBuffer + bufferOffset + bytesRead, remaining);
                }
            }
            else
            {
    ...
    // Render streams: keep the original tone-generator behavior.
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
