...
DRIVER_DISPATCH VmMicDispatchCreateClose;

_Dispatch_type_(IRP_MJ_CLOSE)
DRIVER_DISPATCH VmMicDispatchCreateClose;

_Dispatch_type_(IRP_MJ_DEVICE_CONTROL)
DRIVER_DISPATCH VmMicDispatchDeviceControl;

static NTSTATUS VmMicCreateControlDevice(_In_ PDRIVER_OBJECT DriverObject);
static VOID VmMicDeleteControlDevice();
...
DWORD g_VmMicToneFallback = 0;     // default is silence when no injected data.
UNICODE_STRING g_RegistryPath;      // This is used to store the registry settings path for the driver

static PDEVICE_OBJECT g_VmMicControlDevice = NULL;
static UNICODE_STRING g_VmMicDeviceName = RTL_CONSTANT_STRING(VM_MIC_DEVICE_NAME);
static UNICODE_STRING g_VmMicSymLinkName = RTL_CONSTANT_STRING(VM_MIC_DOSDEVICE_NAME);
...
static NTSTATUS VmMicCreateControlDevice(_In_ PDRIVER_OBJECT DriverObject)
{
    ...
    g_VmMicControlDevice->Flags |= DO_BUFFERED_IO;

    status = IoCreateSymbolicLink(&g_VmMicSymLinkName, &g_VmMicDeviceName);
    if (!NT_SUCCESS(status))
    {
        IoDeleteDevice(g_VmMicControlDevice);
        g_VmMicControlDevice = NULL;
        return status;
    }

    g_VmMicControlDevice->Flags &= ~DO_DEVICE_INITIALIZING;
    return STATUS_SUCCESS;
}
...
static VOID VmMicDeleteControlDevice()
{
    ...
    IoDeleteSymbolicLink(&g_VmMicSymLinkName);
    IoDeleteDevice(g_VmMicControlDevice);
    g_VmMicControlDevice = NULL;
}
...
//=============================================================================
#pragma code_seg()
NTSTATUS VmMicDispatchCreateClose(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp)
{
    if (DeviceObject != g_VmMicControlDevice)
    {
...
//=============================================================================
#pragma code_seg()
NTSTATUS VmMicDispatchDeviceControl(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp)
{
    if (DeviceObject != g_VmMicControlDevice)
    {
        return PcDispatchIrp(DeviceObject, Irp);
    ...
    {
    case IOCTL_VM_MIC_GET_FORMAT:
        if (outputLength < sizeof(VM_MIC_FORMAT) || buffer == NULL)
        {
            status = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        VmMicGetFormat((PVM_MIC_FORMAT)buffer);
        information = sizeof(VM_MIC_FORMAT);
        break;

    case IOCTL_VM_MIC_START:
        VmMicStart();
        break;

    case IOCTL_VM_MIC_STOP:
        VmMicStop();
        break;

    case IOCTL_VM_MIC_WRITE:
        if (inputLength < FIELD_OFFSET(VM_MIC_WRITE_PACKET, payload) || buffer == NULL)
        {
            status = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        {
            PVM_MIC_WRITE_PACKET pkt = (PVM_MIC_WRITE_PACKET)buffer;
            ULONG maxPayload = inputLength - FIELD_OFFSET(VM_MIC_WRITE_PACKET, payload);
            ULONG payload = pkt->payload_bytes;
            if (payload > maxPayload)
            {
                payload = maxPayload;
            }

            payload -= (payload % VM_MIC_BLOCK_ALIGN);
            if (payload == 0)
            {
            ...
            status = VmMicWrite(pkt->payload, payload);
            if (NT_SUCCESS(status))
            {
                information = payload;
            }
    ...
    case IOCTL_VM_MIC_GET_STATS:
        if (outputLength < sizeof(VM_MIC_STATS) || buffer == NULL)
        {
            status = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        VmMicGetStats((PVM_MIC_STATS)buffer);
        information = sizeof(VM_MIC_STATS);
        break;
...

{
    ...
    // QueryRoutine     Flags                                               Name                     EntryContext             DefaultType                                                    DefaultData              DefaultLength
        ...
        { NULL,   RTL_QUERY_REGISTRY_DIRECT | RTL_QUERY_REGISTRY_TYPECHECK, L"VmMicToneFallback",    &g_VmMicToneFallback,   (REG_DWORD << RTL_QUERY_REGISTRY_TYPECHECK_SHIFT) | REG_DWORD, &g_VmMicToneFallback,   sizeof(ULONG)},
#ifdef SYSVAD_BTH_BYPASS
    ...
    DPF(D_VERBOSE, ("VmMicToneFallback: %u", g_VmMicToneFallback));
#ifdef SYSVAD_BTH_BYPASS
...
--*/
    ...
    //
    // Dispatch routines for the virtual mic control device.
    //
    DriverObject->MajorFunction[IRP_MJ_CREATE] = VmMicDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = VmMicDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = VmMicDispatchDeviceControl;
    ...
    //
    // Initialize virtual mic ring buffer and control device.
    //
    ntStatus = VmMicInitialize(VM_MIC_DEFAULT_BUFFER_BYTES);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("VmMicInitialize failed, 0x%x", ntStatus)),
        Done);

    ntStatus = VmMicCreateControlDevice(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("VmMicCreateControlDevice failed, 0x%x", ntStatus)),
        Done);
